(() => {
  ...
  const differences = [{ x: 97, y: 175, r: 14 }, { x: 220, y: 170, r: 16 }];
  let started = false;
  let found = new Set();
  ...
  const drawScene = (ctx, variant) => {
    ...
    if (variant) { ctx.fillStyle = '#fff'; ctx.fillRect(83, 140, 28, 25); ctx.fillStyle = '#2876c7'; ctx.beginPath(); ctx.arc(220, 170, 15, 0, Math.PI * 2); ctx.fill(); }
  };
